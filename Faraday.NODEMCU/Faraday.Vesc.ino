...
void setupVESC()
{
  ...
  vescGetValuesTicker.attach_ms(500, vescGetValues, 0);
}
...
// This is called every 1 seconds by the vescGetValuesTicker()
...
void valuesCallback(mc_values *val) {
  ...
  #endif
   /* Copy updated data */
   vesc_data.v_in = val->v_in;
   vesc_data.current_motor = val->current_motor;
   vesc_data.current_in = val->current_in;
   vesc_data.rpm = val->rpm;
   vesc_data.amp_hours = val->amp_hours;
